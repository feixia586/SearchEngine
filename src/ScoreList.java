...
public class ScoreList {
	...
	public void add_entry(ScoreListEntry entry) {
		scores.add(entry);
	}
		...
		Collections.sort(scores, new Comparator<ScoreListEntry>() {
			public int compare(ScoreListEntry s1, ScoreListEntry s2) {
				if (s1.score != s2.score) {
					...
					try {
						return QryEval.getExternalDocid(s1.docid).compareTo(
								QryEval.getExternalDocid(s2.docid));
					} catch (IOException ex) {
						System.err
								.println("Error: I/O error in getExternalDocid.");
